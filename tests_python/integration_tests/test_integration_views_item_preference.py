...
def test_redirect_to_registration(equal_weight_client):
    ...
    WHEN no participant is logged in and the item selection page is requested
...
def test_render_prefer_item_selection(equal_weight_client, participant_data):
    """
    GIVEN a flask application configured for testing and equal weights
    ...
    with equal_weight_client:
        ...
        response = equal_weight_client.get("/selection/items", data=participant_data)
...
def test_no_render_prefer_item_selection_with_custom_weights(custom_weight_client, participant_data):
    """
    ...
    with custom_weight_client:
        custom_weight_client.post("/register", data=participant_data)
        response = custom_weight_client.get("/selection/items", data=participant_data, follow_redirects=True)
        assert response.status_code == 200
        assert response.request.path == "/rank"


def test_no_render_prefer_item_selection_when_asked_not_to_in_config(participant_data):
    """
    ...
    with app.app_context():
        ...
        response = client.get("/selection/items", data=participant_data, follow_redirects=True)
        assert response.status_code == 200
        assert response.request.path == "/rank"


def test_register_prefer_item_selection(equal_weight_client, participant_data):
    """
    GIVEN a flask application configured for testing and equal weights
    WHEN a participant is logged in and an item preference is posted
    THEN the participant is redirected back to the item preference page to make another preference decision
    """
    with equal_weight_client:
        equal_weight_client.post("/register", data=participant_data)
...
def test_redirect_to_rank_page_when_all_preferences_specified(equal_weight_client):
    """
    GIVEN a flask application configured for testing and equal weights
    WHEN a participant is logged in and the final item preference  for the participants group is posted
    THEN the participant is redirected to the rank page
    """
    participant_id = 1
    # add two more preferences so we only have one remaining in the group
    item_preferences = [
        {'participant_id': participant_id, 'item_id': 10, 'known': True},
        {'participant_id': participant_id, 'item_id': 11, 'known': True},
    ]
